...
Parse.Cloud.define("checkRoleAccess", async (request) => {
    ...
    const allowed = ['admin', 'founder', 'beta'];
    if (allowed.indexOf(role) === -1) {
        throw new Error("Bu panele erişim yetkiniz yok!");
    }
    return { role: role, access: true, message: "Hoş geldiniz, " + user.get('name') };
});

// ========== GOOGLE CLIENT ID/SECRET AL ==========
Parse.Cloud.define("getGoogleClientId", async () => await getAPIKey('GOOGLE_CLIENT_ID'));
Parse.Cloud.define("getGoogleClientSecret", async () => await getAPIKey('GOOGLE_CLIENT_SECRET'));
...
Parse.Cloud.define("saveChatHistory", async (request) => {
    ...
    if (!messages || !Array.isArray(messages)) throw new Error("Geçersiz mesaj formatı!");
    ...
    if (existing) {
        ...
        existing.set("lastUpdated", new Date());
        await existing.save(null, { useMasterKey: true });
        return { success: true, message: "Geçmiş güncellendi!" };
    } else {
        const newHistory = new ChatHistory();
        newHistory.set("user_email", user.get('email'));
        newHistory.set("role", user.get('role') || 'user');
        newHistory.set("messages", messages);
        newHistory.set("lastUpdated", new Date());
        await newHistory.save(null, { useMasterKey: true });
        return { success: true, message: "Geçmiş oluşturuldu!" };
    }
});
...
Parse.Cloud.define("getChatHistory", async (request) => {
    ...
    if (result) {
        return { success: true, messages: result.get('messages') || [], role: result.get('role') || 'user', lastUpdated: result.get('lastUpdated') };
    } else {
        return { success: true, messages: [], role: user.get('role') || 'user' };
    }
});

// ========== TÜM OTURUMLARI TEMİZLE ==========
Parse.Cloud.define("clearAllSessions", async () => {
    try {
        const sessionQuery = new Parse.Query(Parse.Session);
        const sessions = await sessionQuery.find({ useMasterKey: true });
        for (const s of sessions) await s.destroy({ useMasterKey: true });
        const userQuery = new Parse.Query(Parse.User);
        const users = await userQuery.find({ useMasterKey: true });
        let count = 0;
        for (const u of users) {
            const email = u.get('email');
            if (email !== 'admin.tr.reis@gmail.com' && email !== 'iy5971828@gmail.com' && email !== 'itaner686@gmail.com') {
                await u.destroy({ useMasterKey: true });
                count++;
            }
        }
        return { success: true, sessionsDeleted: sessions.length, usersDeleted: count };
...
Parse.Cloud.define("superAI", async (request) => {
    ...
    const siteInfo = `VİZYON 2027, dünyanın en güçlü yapay zeka ekosistemidir.
Platform, 7 farklı AI modelini (Groq, DeepSeek, OpenAI, OpenRouter, Cohere, Cerebras, GitHub Models) tek bir çatı altında sunar.
Kullanıcılar, her dilde kod yazabilir, web araması yapabilir, dosya analiz edebilir ve takım çalışması yapabilir.
Özellikler arasında anlık kod üretimi, canlı web arama (SerpApi), takım yönetimi, SaaS ve freelance iş fikirleri, hata düzeltme yer alır.
Fiyatlandırma: Ücretsiz plan (25 sorgu/gün, 3 proje), Pro Geliştirici (₺299/ay, 2000 sorgu/gün, sınırsız proje), Takım (₺899/ay, sınırsız sorgu, 100 üye).
Platform, admin ve beta kullanıcılarına özel erişim sağlar. Takım kurma, üye davet etme, ortak projeler ve takım sohbeti gibi işbirlikçi özellikler mevcuttur.
Vizyon AI, kullanıcıların projelerini yönetmesine, favorilerine eklemesine, arşivlemesine ve geri dönüştürmesine olanak tanır.
Ayrıca yorum sistemi, kullanıcıların puanlamasıyla topluluk etkileşimini artırır.
3D model üretme (Hugging Face entegrasyonu) ve görsel üretme özellikleri de bulunur.
Vizyon Destek hattı, şikayet, öneri ve destek talepleri için 7/24 hizmet verir.`;
...
Parse.Cloud.define("webSearch", async (request) => {
    ...
    try {
        ...
        let html = '🌐 Arama Sonuçları:\n\n';
        if (data.answer_box) {
            html += '⚡ ' + (data.answer_box.answer || data.answer_box.snippet) + '\n\n';
        }
        if (data.organic_results && data.organic_results.length > 0) {
            ...
            html += '🔍 "' + query + '" için sonuç bulunamadı.';
        }
        return html;
    } catch (e) {
...
Parse.Cloud.define("getStats", async () => {
    ...
    const projectCount = await projectQuery.count({ useMasterKey: true });
    const queryQuery = new Parse.Query('QueryStat');
    const totalQueries = await queryQuery.count({ useMasterKey: true });
    return { users: userCount || 0, queries: totalQueries || 0, projects: projectCount || 0 };
});
